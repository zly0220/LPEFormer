...
def evaluate(model: torch.nn.Module,embedder,criterion,data_loader,add_fea,epoch,args):
    ...
    if args.output_dir and utils.is_main_process():
        output_dir = Path(args.output_dir)
        with (output_dir / "log.txt").open("a") as f:
            f.write("epoch:{:.4f} auc: {:.4f} precision: {:.4f} recall: {:.4f} f1: {:.4f} mcc:{:.4f} auprc:{:.4f} sp:{:.4f} acc: {top1.global_avg:.4f} loss: {losses.global_avg:.4f} \n"
                    .format(epoch, auc_all, pre_score_all, re_score_all, f1, mcc, auprc_all, sp, top1=metric_logger.acc1, losses=metric_logger.loss))
